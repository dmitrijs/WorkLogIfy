...
const App = () => {
    ...
    useEffect(() => {
        window.ipc.on("change.screen", (_$event, where) => {
            store.setScreen(where);
        });

        window.ipc.on("debug.toggle", (_$event, value) => {
            store.toggleDebug(value);
        });

        window.ipc.on("migration", (_$event, what) => {
            if (what === "export") {
        ...
        window.ipc.on("user-is-idle", (_event: Electron.IpcRendererEvent, secondsIdle) => {
            if (timer.isActive()) {
                const timeredTask = store.state.tasks[store.state.taskTimeredId!];
                if (timeredTask && timeredTask.code === "idle") {
                    return; // already idle
                }
                timer.stop(secondsIdle);

                const idleTask = { ...store.getEmptyTask() };
                idleTask.code = "idle";
        ...
        window.ipc.on("user-active-app", (_event: Electron.IpcRendererEvent, activeApp) => {
            if (timer.isActive() && store.state.taskTimeredId) {
                const timeredTask = store.state.tasks[store.state.taskTimeredId];
                console.log("[timeredTask.id, activeAppDescription]", [
                    timeredTask.id,
                    activeApp.appDescription,
                ]);
                store.taskAddActiveApp([
                    timeredTask.id,
                    activeApp.appDescription,
                    activeApp.secondsIdle,
                ]);
            }
            store.addGlobalActiveApp(
                store.state.taskTimeredId,
                activeApp.appDescription,
                activeApp.secondsIdle,
            );
        });

        window.ipc.on("timer-stop", (_event: Electron.IpcRendererEvent) => {
            if (timer.isActive()) {
                timer.stop();
            }
        });

        window.ipc.on("confirm-app-quit", (_event: Electron.IpcRendererEvent) => {
            if (timer.isActive()) {
                timer.stop();
            }
        ...
        window.ipc.on("tasks-menu-command", (_e, params) => {
            ...
            switch (params.command) {
                ...
                case "View in JIRA": {
                    ...
                    const url =
                        "https://" + store.state.settings.jira_host + "/browse/" + task.code;
                    window.open(url);
                    break;
                }
                case "Record to JIRA": {
                    ...
                    const recorded = task.records.reduce(
                        (sum, obj) => sum + obj.recorded_seconds,
                        0,
                    );
                    const step = 6 * 60;
                    ...
                    if (sum < step) {
                        alert(
                            "Time to record is too little. At least " +
                                timespanToText(step) +
                                " are required.",
                        );
                        return;
                    ...
                    const timeSpentSeconds = Math.ceil(sum / step) * step; // round up

                    const taskCode = task.code;
                    const timeStarted = moment(task.sessions[0].started_at);
                    const taskDate = moment(task.date, "YYYY-MM-DD");
                    if (taskDate.format("YYYY-MM-DD") === task.date) {
                        // valid date
                        timeStarted.year(taskDate.year());
                        timeStarted.month(taskDate.month());
                        timeStarted.date(taskDate.date());
                    }
                    const workLogTime = timeStarted.format(JIRA_TIME_FORMAT);

                    // TODO: `request-promise` was replaced with `fetch`, these options were not adjusted
                    const options = {
                        url:
                            "https://" +
                            store.state.settings.jira_host +
                            "/rest/api/2/issue/" +
                            taskCode +
                            "/worklog?notifyUsers=false&adjustEstimate=auto",
                        auth: {
                    ...
                    if (store.state.is_debug) {
                        alert(
                            "Would be sent to JIRA: " +
                                timespanToText(timeSpentSeconds) +
                                " at " +
                                workLogTime,
                        );
                        jiraResponseWorkLog = { response: {} };
                    } else {
                        jiraResponseWorkLog = window.ipc.sendSync(
                            "jira.request",
                            cloneDeep(options) as any,
                        );
                    }
                    ...
                    if (jiraResponseWorkLog.error) {
                        ...
                        store.taskAddRecordedSeconds([
                            task.id,
                            timeSpentSeconds,
                            jiraResponseWorkLog.response.id || null,
                        ]);
    ...
    return (
        <div
            className={`App ${store.state.is_debug ? "isDebug" : ""} dark:bg-zinc-800 dark:text-neutral-300`}
        >
            <div className="AppScreen">
                ...
                <React.Fragment>
                    ...
                    {store.state.screen === "calendar" && <CalendarWindow weekKey={""} />}
            ...
            {store.state.is_debug && (
                <div className="Debug">
                    ...
                    {Object.entries(store.state).map(([key, value]) =>
                        ...
                        key === "tasks" ||
                        key === "activeApps" ||
                        key === "templates" ? null : key === "taskInClipboard" ? (
                            <div key={key}>
                                <strong>{key} (redacted):</strong>{" "}
                    ...
                    <hr />
                    Integrations:
                    <button type="button" className="btn btn-secondary btn-xs" onClick={lock}>
                        lock
                    </button>
                    <button type="button" className="btn btn-secondary btn-xs" onClick={wakeup}>
                        wake up
                    </button>
                    <br />
                    Timer:
                    <button
                        ...
                        onClick={timerStop}
                        disabled={!store.state.taskTimeredId}
                    >
                        stop
                    </button>
                    <hr />
                    ...
                    <button
                        type="button"
                        className="btn btn-secondary btn-xs"
                        onClick={() => progressBar({ indeterminate: true })}
                    >
                        ∞
                    </button>
                    <button
                        type="button"
                        className="btn btn-secondary btn-xs"
                        onClick={() => progressBar({ indeterminate: false })}
                    >
                        -
                    </button>
                    <button
                        type="button"
                        className="btn btn-secondary btn-xs"
                        onClick={() => progressBar({ progress: 0.01 })}
                    >
                        1%
                    </button>
                    <button
                        type="button"
                        className="btn btn-secondary btn-xs"
                        onClick={() => progressBar({ progress: 0.7 })}
                    >
                        70%
                    </button>
                    <hr />
