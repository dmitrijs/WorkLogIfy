...
function populateSubtaskIds(state) {
    ...
    for (const task of Object.values(state.tasks) as any[]) {
        if (task.parentId) {
            subtasks[`${task.date}/${task.parentId}`] =
                subtasks[`${task.date}/${task.parentId}`] || [];
...
type StoreMethodsType = {
    ...
    calendarHoveredDayCode: (dayCode: string | null) => void;
    saveTasks: () => void;
...
export type StoreType = { state: typeof initialState } & StoreMethodsType;
...
const StoreContentProvider = ({ children }: any) => {
    ...
    function addActiveApp(
        ...
        const activeApps = state.tasks[task_id].activeApps;
        activeApps.push({
    ...
    function addRecord(
        ...
        const records = state.tasks[task_id].records;
        const record = {
    ...
    const storeMethods: StoreMethodsType = {
        async upsertTasks(tasks = null) {
            ...
            const tsks: { date: any; uid: string; code: string; raw: TaskObj }[] = [];
            for (const task of Object.values<TaskObj>(tasks || state.tasks)) {
                const tsk = {
                    date: state.day_key,
                    uid: task.id,
                    code: task.code,
        ...
        getEmptyTask(): TaskEditedObj {
            ...
            let refTask = null as TaskObj | null;
            let refRootTask = null as TaskObj | null;
            ...
            if (refTask && state.creatingByExtract) {
                ...
                let taskSpentSeconds = refTask.sessions.reduce(
                    (sum, obj: SessionObj) => sum + obj.spent_seconds,
                    0,
                );
        ...
        createTask(task) {
            ...
            if (task.time_add_minutes) {
                const spentSeconds = parseInt(task.time_add_minutes) * 60 || 0;
            ...
            if (task.time_add_idle_seconds) {
                const seconds = parseInt(task.time_add_idle_seconds) || 0;
            ...
            if (isArray(state.tasks)) {
                alert(
                    "ASSERT FAILED: `tasks` has invalid data type. Tasks might not be persisted.",
                );
            }
        ...
        saveTask(task: TaskEditedObj) {
            ...
            if (task.time_record_minutes) {
                const recordSeconds = parseInt(task.time_record_minutes) * 60 || 0;
        ...
        async setDay(day: string, supabaseState = "") {
            ...
            state.day_key_prev_week = moment(day, "YYYY-MM-DD")
                .startOf("isoWeek")
                .subtract(1, "week")
                .format("YYYY-MM-DD");
            state.day_key_next_week = moment(day, "YYYY-MM-DD")
                .endOf("isoWeek")
                .add(1, "day")
                .format("YYYY-MM-DD");
            ...
            console.log(
                "setDay supabaseState",
                supabaseState,
                useSupabaseSettings.getState().state,
            );
            if ((supabaseState || useSupabaseSettings.getState().state) === "enabled") {
                const tasksRaw = await supabase
                    .from("tasks")
                    .select("*")
                    .eq("date", day)
                    .order("raw->>time_spent_seconds", { ascending: false });
                const tasks = workday.tasks || {};
                for (const taskRaw of tasksRaw.data || []) {
        ...
        setDayFromJson(tasksJson: string) {
            ...
            try {
                ...
                console.log("tasksJson could not be parsed:", ex);
            }
            if (
                !tasks ||
                !Object.keys(tasks).length ||
                !Object.keys(tasks)[0].match(/^task_\d+$/)
            ) {
        ...
        clipboardPaste() {
            ...
            const id = "task_" + moment.utc();
            const newTask = { ...state.taskInClipboard, id: id };
        ...
        activeTimer(secondsElapsed) {
            ...
            if (secondsElapsed - state.lastSavedElapsedSeconds >= 60) {
                // save every minute
        ...
        calendarHoveredDayCode(dayCode: string | null) {
        ...
        loadAsanaTasks(force = false) {
            ...
            const asanaTasksCall = window.ipc.sendSync("jira.request", {
                url:
                    `https://app.asana.com/api/1.0/workspaces/${state.settings.asana_workspace_id}/tasks/search?` +
            ...
            const completedOnAfter = moment().subtract(2, "months").format("YYYY-MM-DD");
            ...
            const asanaTasksCompletedCall = window.ipc.sendSync("jira.request", {
                url:
                    ...
                    "&resource_subtype=default_task" +
                    "&assignee.any=me" +
                    "&completed=true&completed_on.after=" +
                    completedOnAfter +
                    "&is_subtask=false" +
                    (state.settings.asana_extra_filter || ""),
                headers: {
            ...
            state.asanaTasks = keyBy(
                [...asanaTasksCall.response.data, ...asanaTasksCompletedCall.response.data],
                "gid",
            );
            storeMethods.updateState({
        ...
        loadYoutrackTasks(force = false) {
            ...
            const youtrackTasksCall = window.ipc.sendSync("jira.request", {
                url:
                    `https://${state.settings.youtrack_domain}/api/issues?fields=id,idReadable,description,summary,customFields(name,value(name))` +
                    "&query=" +
                    encodeURIComponent(state.settings.youtrack_query || ""),
                headers: {
        ...
        dragContinue($event) {
            ...
            if (!state.drag.readyToDrop) {
                ...
                const coefficient = 10.0 / Math.log10(distance);
        ...
        dropTime(event, task_id) {
            ...
            if (
                Math.abs(state.drag.minutes) > 0 &&
                state.drag.taskFrom &&
                state.drag.taskTo &&
                state.drag.taskFrom !== state.drag.taskTo
            ) {
                if (state.drag.taskTo.startsWith("universe")) {
                    storeMethods.taskAddSession([
                        state.drag.taskFrom,
                        -state.drag.minutes,
                        "state.drag",
                    ]);
    ...
    useEffect(() => {
        async function initialize() {
            if (!state.initialized) {
                ...
                storeMethods.setDay(today.format("YYYY-MM-DD"), supabaseState);
    ...
    const contextValue = useMemo<StoreType>(
        (): StoreType => ({
            state: state,
            ...storeMethods,
        }),
