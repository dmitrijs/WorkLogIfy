import { StoreType } from "@/renderer/Store/Store";
...
let localState = {} as StoreType["state"];

export function build_sort_value(
    task: TaskObj & {
        task_not_started: boolean;
        last_session: SessionObj | null;
        first_session: SessionObj | null;
        is_timered: boolean;
    },
) {
...
class TasksSorter {
    ...
    constructor(tasks: Record<string, TaskObj>) {
        Object.values(tasks).forEach((task) => {
            ...
            if (started_at) {
                this.codeToLastSession[code] = Math.max(
                    this.codeToLastSession[code] ?? -1,
                    started_at,
                );
            }
        });
    ...
    enhanceTask(task: TaskObj) {
        ...
        let last_session = null as SessionObj | null;
        let first_session = null as SessionObj | null;
        if (task.sessions && task.sessions.length > 0) {
            task_not_started = false;
            last_session = task.sessions[task.sessions.length - 1];
            first_session = task.sessions[0];
        }
        const is_timered = localState.taskTimeredId === task.id;
    ...
    compare(task1: TaskObj, task2: TaskObj, forReport = false) {
        ...
        if (localState.settings.sorting_order === "time_created_reversed") {
            const sortOrder = -task1.created_at.localeCompare(task2.created_at);

            return (
                (localState.tasksHideUnReportable || localState.tasksShowAsReport ? -1 : 1) *
                sortOrder
            );
        }
...
export function sort_tasks(
    tasks: Record<string, TaskObj>,
    forReport = false,
): Record<string, TaskObj> {
    const sorter = new TasksSorter(tasks);

    return Object.fromEntries(
        Object.entries(tasks).sort((task1: [string, TaskObj], task2: [string, TaskObj]) => {
...
export function Store_MergeSameCodes(tasks: TaskObj[], mergeOnlySubtasks = true) {
    ...
    tasks.forEach((task: TaskObj) => {
        ...
        const mergingCode = mergeOnlySubtasks ? task.code : task.code.replace(/-.*/, "");
        const existing = unique[mergingCode];
        if (!existing) {
        ...
        if (existing.time_charge_seconds <= 0) {
            // replace empty
...
export default function Store_GetGroupedTasks(state): Record<string, TaskGroupObj> {
    ...
    const tasksList = [] as TaskObj[];
    Object.values(state.tasks).forEach((originalTask, _key) => {
        const task = cloneDeep(originalTask) as any;
        task.time_charge_text = "error";

        task.time_spent_seconds = task.sessions.reduce(
        ...
        task.time_recorded_seconds = task.records.reduce(
            (sum, obj: RecordObj) => sum + obj.recorded_seconds,
            0,
        );
    ...
    let groups = mapValues(tasks, (tasksList, _date) => {
        let spent = 0;
        let charge = 0;
        let distributed = 0;
        ...
        tasksList.forEach((task) => {
            ...
            time_recorded += task.time_recorded_seconds || 0;
        ...
        } as unknown as TaskGroupObj;
    });

    // populate charge_time
    groups = mapValues(groups, (group, group_key) => {
        const distributed = group.time_distributed_seconds;
        const not_distributed = group.time_not_distributed_seconds;
        const frozen = group.time_frozen_seconds;
        ...
        let tasks = map(group.tasks, (task) => {
            ...
            if (!task.chargeable || task.distributed) {
                ...
                const spent = parseInt(String(task.time_spent_seconds));

                task.time_charge_seconds = spent + (spent / not_distributed) * distributed;
            }

            task.time_charge_text = timespanToText(task.time_charge_seconds || 0);
            return task;
        });
        ...
        tasks = tasks.map((task) => {
            task.time_charge_seconds = applyRoundingMinutes(
                task.time_charge_seconds || 0,
                state.settings.rounding_minutes,
            );
        ...
        {
            const secondsMissing = group.time_charge_seconds - time_charge_rounded_seconds;
            const microTimeBlockSeconds = 60 * 5;
            const blockCount = Math.round((secondsMissing + 60) / microTimeBlockSeconds);
            const secondsMissingRounded = blockCount * microTimeBlockSeconds;

            // chargest task
            let chargest_task_seconds = 100000000;
            let chargest_task_id = null as string | null;

            tasks.forEach((task) => {
                if (
                    !task.frozen &&
                    task.time_charge_seconds &&
            ...
            if (Math.abs(secondsMissing) > 60) {
                tasks.forEach((task) => {
                    if (task.id === chargest_task_id) {
                        ...
                        task.time_charge_seconds =
                            (task.time_charge_seconds || 0) + secondsMissingRounded;
                        task.time_charge_text = timespanToText(task.time_charge_seconds!);
                    }
                    return task;
                });
    ...
    return Object.fromEntries(
        Object.entries(groups).sort(
            (group1: [string, TaskGroupObj], group2: [string, TaskGroupObj]) => {
