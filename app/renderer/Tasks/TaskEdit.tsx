...
const TaskEdit = ({ mode }: { mode: string }) => {
    ...
    useEffect(() => {
        ...
        if (
            mode === "new" ||
            (store.state.settings.youtrack_enabled &&
                !Object.keys(store.state.youtrackTasks || {}).length)
        ) {
            (async () => {
    ...
    const codeChanged = () => {
        ...
        const matches = task.code.match(
            /(?:https:\/\/)?[^.]+\.atlassian\.net\/browse\/(\S+-\d+).*?/,
        );
        if (matches) {
            ...
            >
                <table style={{ width: "100%" }}>
                    <tbody>
                                            ...
                                            {store.state.templates.map((template) => (
                                                ...
                                                >
                                                    ...
                                                    <div style={{ float: "right" }}>
                                                        <i
                                                            className={`IconAsInput icofont-not-allowed ${!template.chargeable ? "active" : ""}`}
                                                        ></i>
                                                        <i
                                                            className={`IconAsInput icofont-exchange ${template.distributed ? "active" : ""}`}
                                                        ></i>
                                                        <i
                                                            className={`IconAsInput icofont-unlock ${template.frozen ? "active" : ""}`}
                                                        ></i>
                                                    </div>
                                                </div>
                                            ))}
                        ...
                        {store.state.settings.asana_enabled && (
                            <tr>
                                <td>
                                    Asana task (
                                    <a
                            ...
                            <td className="Complex">
                                <div style={{ marginRight: "11px" }}>
                                    <div className={"flex gap-1"}>
                                        ...
                                        <ComboboxProjects
                                            projects={store.getTaskProjects()}
                                            currentProject={task.code}
                                            onChange={(code) => {
                                    ...
                                    <span>
                                        ...
                                        <input
                                            ...
                                            value={task.source || ""}
                                            onChange={(e) =>
                                                setTask({ ...task, source: e.target.value })
                                            }
                                        />
                                    </span>
                                </div>
                                ...
                                <div>
                                    <span>
                                        <input
                                            ...
                                            onChange={(e) =>
                                                setTask({ ...task, group_key: e.target.value })
                                            }
                                        />
                                    </span>
                                    <span
                                        style={{
                                            fontWeight: task.taskIdExtractedFrom ? "bold" : "",
                                        }}
                                    >
                                        ...
                                        <input
                                            type="text"
                                            className="narrow"
                                            value={task.time_add_minutes || ""}
                                            onChange={(e) =>
                                                setTask({
                                                    ...task,
                                                    time_add_minutes: e.target.value,
                                                })
                                            }
                                        />
                                        m
                                ...
                                <div>
                                        ...
                                        {task.date !== store.state.day_key && (
                                            <button
                                                ...
                                                onClick={() =>
                                                    setTask({
                                                        ...task,
                                                        comment: [task.date, task.comment]
                                                            .filter((s) => !!s)
                                                            .join(", "),
                                    ...
                                    <span>
                                        ...
                                        <input
                                            type="text"
                                            className="narrow"
                                            value={task.time_record_minutes || ""}
                                            onChange={(e) =>
                                                setTask({
                                                    ...task,
                                                    time_record_minutes: e.target.value,
                                                })
                                            }
                                        />
                                        m
                        ...
                        <tr
                            className={
                                task.parentId && store.parentIsMissing(task) ? "HasError" : ""
                            }
                        >
                            <td>Parent task:</td>
                            <td>
                        ...
                        <tr>
                            <td colSpan={2}>
                                ...
                                <div className="btn-group float-right" role="group">
                                    <i
                                        className={`TaskFlagIconAsInput icofont-not-allowed ${!task.chargeable ? "active" : ""}`}
                                        onClick={() =>
                                            setTask({ ...task, chargeable: !task.chargeable })
                                        }
                                    ></i>
                                    <i
                                        className={`TaskFlagIconAsInput icofont-exchange ${task.distributed ? "active" : ""}`}
                                        onClick={() =>
                                            setTask({ ...task, distributed: !task.distributed })
                                        }
                                    ></i>
                                    <i
                                        className={`TaskFlagIconAsInput icofont-unlock ${task.frozen ? "active" : ""}`}
                                        onClick={() => setTask({ ...task, frozen: !task.frozen })}
                                    ></i>
                                    <button
                                        className={`btn btn-secondary btn-sm ${store.state.taskTimeredId === task.id ? "btn-primary" : ""}`}
                                    >
