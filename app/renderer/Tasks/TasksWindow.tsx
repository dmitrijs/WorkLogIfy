...
const TasksWindow = () => {
    ...
    const [tasksGroupedAndMerged, setTasksGroupedAndMerged] = useState({});
    ...
    useEffect(() => {
        ...
        const contextMenuShow = (e) => {
            store.selectHovered();
            e.preventDefault();
            createMenu().popup({ window: window.remote.getCurrentWindow() });
        };
    ...
    const computeTotal = () => {
        ...
        for (let group of Object.values(tasksGrouped)) {
    ...
    const copyToClipboardAllTasks = (ev) => {
        ...
        for (let group of Object.values(tasksGroupedAndMerged)) {
    ...
    return (
        <div className="TasksWindow" onMouseMove={dragContinue} onMouseUp={dragStop}>
            ...
            <div className={`TasksTable ${store.state.tasksShowAsReport ? 'ShowAsReport' : ''} ${store.state.tasksHideUnReportable ? 'ShowCompact' : ''}`} key={forceUpdateKey.current} onClick={() => store.deselectAll()}>
                ...
                {Object.entries(tasksGrouped).map(([date, group]) => (
                    <div className={`TRowDate ${tasksGrouped.length > 1 ? 'SubTotal' : ''} ${group.erroneous ? 'Erroneous' : ''} ${store.state.tasks[store.state.taskTimeredId]?.date === date ? 'TimeredGroup' : ''}`} key={date}>
                        ...
                        {group.tasks.map((task, task_id) => (
                            !task.parentId || store.parentIsMissing(task) ? (
                                <TaskRow key={task.id} tasksGrouped={tasksGrouped} group_id={date} task_id={task_id} onDragStart={dragStart} />
                            ) : null
                        ))}
                    </div>
                ))}
            ...
            <div>
                <textarea className="GlobalNotes" rows="1" ref={globalNotesInputRef} onBlur={handleBlur}>
