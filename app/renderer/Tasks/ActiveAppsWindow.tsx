            ...
            <table>
                ...
                <tbody>
                    {activeApps.map((activeApp) => (
                        <tr
                            key={activeApp.noticed_at}
                            className={activeApp.seconds_idle > 60 ? "Idle" : ""}
                        >
                            <td className="TD-Time" title={`Idle: ${activeApp.seconds_idle}s`}>
                                ...
                                {moment().diff(moment(activeApp.noticed_at), "hour") <= 4 && (
                                    <span>
                                        ({moment().diff(moment(activeApp.noticed_at), "minute")}m
                                        ago)
                                    </span>
                                )}
                            </td>
                            <td>
                                {activeApp.timered_task
                                    ? store.state.tasks[activeApp.timered_task]?.code || "Y"
                                    : "-"}
                            </td>
