...
const TaskRow = ({ tasksGrouped, group_id, task_id, onDragStart }) => {
    ...
    const rootTasks = Object.values(tasksGrouped).flatMap(group => Object.values(group.tasks)).filter(t => t.code !== 'idle' && !t.parentId);
    ...
    return (
        <div
            ...
            onClick={rowOnClick}
        >
            <div className="TRowContent" style={{ display: (store.state.tasksHideUnReportable && (task.distributed || !task.chargeable)) && store.state.taskTimeredId !== task.id ? 'none' : 'block' }}>
                ...
                <div className="TCol --timespan" onClick={(event) => dropTime(event, task)} onMouseDown={(event) => { event.preventDefault(); onDragStart(event, task); }} title={`Final charge: ${task.time_charge_text}\nRecorded: ${task.time_recorded_text}\nNot recorded: ${task.time_unrecorded_text}`}>
                    <span className="--timespan-spent">
                        {store.state.taskTimeredId === task.id ? `${timespanToText(store.state.timerElapsedSeconds)}<br />` : null}
            ...
            <div className="Subtasks">
                {task.subtaskIds.map(subtask_id => (
                    <TaskRow
                        key={subtask_id}
                        group_id={group_id}
                        task_id={subtask_id}
