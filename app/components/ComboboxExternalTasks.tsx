            ...
            <PopoverTrigger asChild className={"!px-1"}>
                ...
                >
                    <span>
                        {currentTask ? (
                            <>
                                [{currentTask.idReadable}] {currentTask.summary}
                            </>
                        ) : currentTaskCode ? (
                            "Current: " + currentTaskCode
                        ) : (
                            ""
                        )}
                    </span>
                ...
                >
                    ...
                    <CommandList>
                        ...
                        <CommandGroup forceMount={true}>
                            {currentTaskCode && !currentTask && (
                                <CommandItem>
                                    <span>
                                        <b>Current:</b> {currentTaskCode}
                                    </span>
                                </CommandItem>
                            )}
                        ...
                        {Object.entries(tasksGrouped).map(([groupName, tasks]: [string, any]) => (
                            <CommandGroup
                                key={groupName}
                                heading={
                                    <strong className={priorityToTextClassName[groupName]}>
                                        {groupName}
                                    </strong>
                                }
                                className={priorityToClassName[groupName] || ""}
                            >
                                {tasks.map((task) => (
                                    <CommandItem
                                        value={`${task.idReadable}|${task.summary}`}
                                        key={task.idReadable}
                                        onSelect={onSelect}
                                    >
                                        <span className={"flex gap-1 items-center"}>
                                            {stateIcon(task.State)} [{task.idReadable}]{" "}
                                            {task.summary}
                                        </span>
                                    </CommandItem>
                                ))}
