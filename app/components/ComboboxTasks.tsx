            ...
            <PopoverTrigger asChild className={"!px-1"}>
                ...
                >
                    <span>
                        {currentTask ? (
                            <>
                                {currentTask.code && `[${currentTask.code}]`} {currentTask.title}{" "}
            ...
            <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0">
                ...
                >
                    ...
                    <CommandList>
                        ...
                        {Object.values(tasks || {})
                            .filter((task: TaskObj) => !excludeIds.includes(task.id))
                            .map((parentTask: TaskObj) => (
                                <>
                                    {parentTask.id !== currentTask?.id && !parentTask.parentId && (
                                        <CommandItem
                                            value={`${parentTask.id}|${parentTask.code}`}
                                            key={parentTask.id}
                                            onSelect={onSelect}
                                        >
                                            <span
                                                ...
                                                style={{
                                                    color:
                                                        parentTask.distributed ||
                                                        !parentTask.chargeable
                                                            ? "#595959"
                                                            : "",
                                                }}
                                            >
